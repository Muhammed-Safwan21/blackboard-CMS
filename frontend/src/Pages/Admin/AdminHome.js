...
const {data:notices,isLoading:loadingNotice,error} = useGetAllNoticesQuery()
  ...
  return (
    ...
    {notices?.length > 0 ? (
      <>
      {notices.map((notice)=>(
       <div key={notice._id} className='bg-gray-100 rounded border border-gray-300 m-3'>
       <div className='flex p-2 justify-around items-center bg-blue-100 text- font-medium text-center border border-gray-300'>
